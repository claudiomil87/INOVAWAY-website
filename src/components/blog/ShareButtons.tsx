...
export default function ShareButtons({ title, url }: ShareButtonsProps) {
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <span className="text-sm text-white/50">{t("sharePost")}:</span>
      {shares.map((share) => (
        <a
          key={share.name}
          href={share.href}
      ...
      <button
        ...
        className="flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-200 hover:scale-110"
        style={{
          background: copied ? "rgba(0,255,65,0.1)" : "rgba(255,255,255,0.05)",
          border: copied ? "1px solid rgba(0,255,65,0.3)" : "1px solid rgba(255,255,255,0.1)",
